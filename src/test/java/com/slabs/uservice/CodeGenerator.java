...
import static com.slabs.uservice.core.ProjectConstant.*;

/**
 * 代码生成器,根据数据表名称生成Model、Mapper、Service、Controller
 *
...
public class CodeGenerator {
    ...
    // JDBC配置，请修改为你项目的实际配置
    private static final String JDBC_URL = "jdbc:mysql://localhost:3306/test" + "?useUnicode=true&characterEncoding=utf-8&useLegacyDatetimeCode=false&serverTimezone=UTC";
    ...
    // @author
    private static final String AUTHOR = "BingYe";
    // @date
    private static final String DATE = new SimpleDateFormat("yyyy/MM/dd").format(new Date());

    // 通过overwrite判断是否覆盖对应目录下的相关文件
    ...
    public static void main(String[] args) {
        ...
        System.out.println("可能以存在相关文件,是否覆盖? y/n:");
        if (scanner.next().equals("y")) {
            overwrite = true;
        }

        genCode("permission","role","role_permission","user","user_role");
    ...
    private static void genModelAndMapper(String tableName, String modelName) {
        ...
        /**
         * Context用于指定生成一组对象的环境,例如: 指定要连接的数据库,要生成对象的类型和要处理的数据库中的表
         * 1. ModelType.FLAT:           该模型为每张表只生成一个实体类,这个实体类包含了表中的所有字段.(这种模型最简单)
         * 2. ModelType.HIERARCHICAL    如果表有主键,那么该模型会产生一个单独的主键实体类,如果表还有Blob字段,则会生成一个包含所有
         * 字段,则会生成一个包含Blob字段的单独实体类,然后为其他的字段生成一个单独的实体类,MBG会在所有生成的实体类之间维护一个继承关系.
         * 3. ModelType.CONDITIONAL     这个是默认值,这个模型(2)类似,除了如果那个单独的类将只包含一个字段,将不会生成一个单独的类,
         * 因此,如果一个表的主键只有一个字段,那么不会为该表生成单独的实体类,会将该字段合并到基本的实体类中.
         *
         */
